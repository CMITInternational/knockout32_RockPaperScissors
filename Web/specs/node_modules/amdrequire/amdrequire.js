var Module = require('module'),
	...
	noderequire = function(dep, thisModule){
	    ...
	    console.log('noderequire: dep = [' + dep + ']');
	...
	amdrequire = function(deps, callback, thisModule){
		...
		deps.forEach(function(depName){
		    ...
		    console.log('amdrequire: depPath = [' + depPath + ']');
	...
	resolveAmdPath = function(request, thisModule) {
	    ...
	    console.log('resolveAmdPath: request = [' + request + ']');

	    var start = request.substring(0, 2),
			filepath
		;

	    console.log('resolveAmdPath: start = [' + start + ']');
	    ...
	    if (request[0] == '/') {
	        ...
	        var absPath = Module._findPath(requestName, lookupPaths);

	        console.log('resolveAmdPath: absolute absPath = [' + absPath + ']');
		...
		if (start !== './' && start !== '..' && start !== '.\\'){
			if(paths[request]){
			    //Named require
			    var existingPath = paths[request];

			    console.log('resolveAmdPath: relative existingPath = [' + existingPath + ']');
			...
			;
			// Let node look up the path for us
			var findPath = Module._findPath(requestName, lookupPaths);

			console.log('resolveAmdPath: relative findPath = [' + findPath + ']');
		...
		filepath = win ? request.replace(/\//g, '\\') : request;
		var resolve = path.resolve(pathStack[pathStack.length - 1], filepath);

		console.log('resolveAmdPath: resolve = [' + resolve + ']');
